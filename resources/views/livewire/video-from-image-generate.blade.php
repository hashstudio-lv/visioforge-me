    ...
    <div class="rounded-3xl bg-white px-4 py-12 dark:bg-gray-dark lg:px-8">
        ...
        @if(!$externalId)
            <div class="relative">
                ...
                <select
                    class="h-14 w-full appearance-none rounded-[20px] border-2 border-gray/20 bg-transparent py-3 font-semibold outline-none ltr:pl-4 ltr:pr-10 rtl:pr-4 rtl:pl-10">
            ...
            <div class="mt-10">
                <input type="file" id="fileUpload" name="file" class="hidden" wire:model="file"/>
                <label for="fileUpload"
                       class="btn block w-full cursor-pointer bg-gray px-12 py-4 text-center capitalize text-white dark:bg-white dark:text-black dark:hover:bg-secondary">
                    Upload File
                </label>
            ...
            @else
                ...
                @if(!$externalId)
                    <div class="mt-10 text-center ltr:lg:text-right rtl:lg:text-left">
                        ...
                        @else
                            <button
                                class="cursor-pointer btn px-12 capitalize text-white dark:text-black"
                                disabled
                                style="cursor: pointer; background-color: rgba(119, 128, 161, 0.33)"
                            >
                                {{ __('Generate') }}
                            </button>
                        @endif
