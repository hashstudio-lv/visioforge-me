        ...
        <div class="container">
            <div class="items-center justify-between py-10 md:flex md:h-[400px] md:py-0">
                <div class="heading relative mb-0 text-center ltr:md:text-left rtl:md:text-right">
                    <h4 class="!text-white">{{ $title ?? __('Dashboard') }}</h4>
                    <div class="text-white mt-2">{{ __('Your balance:') }} <span
                            class="text-primary font-bold">{{ __(':amount VCT', ['amount' => auth()->user()->balance]) }}</span>
                    </div>
                    <div class="mt-4">
                        <ul class="filters realestate-filter flex flex-row gap-5 whitespace-nowrap pb-2.5 sm:gap-[30px]">
                            <li class="@if(in_array(request()->route()->getName(), ['dashboard', 'dashboard'])) bg-primary text-white @endif">
                                <a href="{{ route('dashboard') }}" class="btn rounded-none bg-gray/[0.08] hover:bg-primary @if(in_array(request()->route()->getName(), ['dashboard', 'dashboard'])) text-white @else text-gray @endif">{{ __('My creatives') }}</a>
                            </li>
                            <li class="@if(in_array(request()->route()->getName(), ['deposits.index', 'deposits.create'])) bg-primary text-white @endif">
                                <a href="{{ route('deposits.index') }}" class="btn rounded-none bg-gray/[0.08] hover:bg-primary @if(in_array(request()->route()->getName(), ['deposits.index', 'deposits.create'])) text-white @else text-gray @endif">{{ __('My Deposits') }}</a>
                            </li>
                        </ul>
                    </div>
                </div>
