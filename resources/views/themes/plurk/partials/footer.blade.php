<footer class="mt-auto bg-white dark:bg-transparent dark:bg-gradient-to-b dark:from-white/[0.03] dark:to-transparent">
    <div class="container">
        <div class="grid gap-y-10 gap-x-4 py-14 sm:grid-cols-3 lg:grid-cols-5 lg:py-[100px]">
            <div class="relative">
                <a href="{{ route('home') }}" class="text-primary font-bold uppercase text-lg">
                    Video Creator <span class="text-black text-xs">PRO</span>
                </a>
                ...
                <img src="{{ asset('assets/images/footer-shape.png') }}" alt="footer-shape" class="absolute bottom-0 right-0 sm:left-0" />
                <img
                    src="{{ asset('assets/images/footer-shape-dark.png') }}"
            ...
            <div>
                <ul class="flex flex-col gap-3 font-bold">
                    ...
                    <li><a href="{{ route('pages.show') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('Home') }}</a></li>
{{--                    <li><a href="{{ route('pages.show', 'about-us') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('About us') }}</a></li>--}}
                    <li><a href="{{ route('pages.show', 'contact-us') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('Contact us') }}</a></li>
                </ul>
            </div>
            <div>
                <ul class="flex flex-col gap-3 font-bold">
                    <li class="mb-3 text-lg font-extrabold text-black dark:text-white">{{ __('Policies') }}</li>

                    <li><a href="{{ route('pages.show', 'terms-and-conditions') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('Terms & Condition') }}</a></li>
                    <li><a href="{{ route('pages.show', 'privacy-policy') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('Privacy Policy') }}</a></li>
                    <li><a href="{{ route('pages.show', 'cookie-policy') }}" class="inline-block transition hover:scale-110 hover:text-secondary">{{ __('Cookie Policy') }}</a></li>
                </ul>
            </div>
            <div>
                <ul class="flex flex-col gap-3 font-bold">
                    <li class="mb-3 text-lg font-extrabold text-black dark:text-white">{{ __('Languages') }}</li>
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <li>
                            <a href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" class="inline-block transition capitalize hover:scale-110 hover:text-secondary">
                                {{ $properties['native'] }}
                            </a>
                        </li>
                    @endforeach
            ...
            <div>
                <ul class="flex flex-col gap-3 font-bold">
                    ...
                    <li>{{ env('COMPANY_NAME') }}</li>
                    <li>{{ env('COMPANY_ADDRESS') }}</li>
                    <li>
                        <a href="mailto:{{ env('COMPANY_EMAIL') }}" class="inline-block transition hover:scale-110 hover:text-secondary">
                            {{ env('COMPANY_EMAIL') }}
                        </a>
                    </li>
                </ul>
    ...
    <div class="bg-gradient-to-r from-[#FCF1F4] to-[#EDFBF9] py-5 dark:border-t-2 dark:border-white/5 dark:bg-none">
        <div class="container flex justify-between items-center">
            ...
            <div>
            <span class="curr-year">{{ now()->year }}</span>
